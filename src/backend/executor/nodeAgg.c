...
static TupleTableSlot **agg_retrieve_direct_qualtuple(AggState *aggstate);


static void agg_fill_hash_table(AggState *aggstate);


static void agg_fill_hash_table_listfull(AggState *aggstate);

static void agg_fill_hash_table_qualtuple(AggState *aggstate);


static TupleTableSlot *agg_retrieve_hash_table(AggState *aggstate);


static TupleTableSlot **agg_retrieve_hash_table_listfull(AggState *aggstate);

static TupleTableSlot **agg_retrieve_hash_table_qualtuple(AggState *aggstate);
...
TupleTableSlot **
ExecAggListQualTuple(AggState *node)
{
	...
	if (((Agg *) node->ss.ps.plan)->aggstrategy == AGG_HASHED)
	{
		if (!node->table_filled)
			agg_fill_hash_table_qualtuple(node);
		return agg_retrieve_hash_table_qualtuple(node);
	}
	else
		return agg_retrieve_direct_qualtuple(node);
}
...
ExecAggListFull(AggState *node)
{
	...
	{
		if (!node->table_filled)
			agg_fill_hash_table_listfull(node);
		return agg_retrieve_hash_table_listfull(node);
	}
	else
...
TupleTableSlot **resultlist; //[MYTODO] global
int resultposagg;

static TupleTableSlot **
agg_retrieve_direct_qualtuple(AggState *aggstate)
{
	Agg		   *node = (Agg *) aggstate->ss.ps.plan;
	PlanState  *outerPlan;
	...
	while (!aggstate->agg_done)
	{
		...
		if (ExecQual(aggstate->ss.ps.qual, econtext, false))
		{
			...
			result = ExecProjectBuffer(aggstate->ss.ps.ps_ProjInfo, &isDone,mybuffersize - resultposagg);//Taras: [MYTODO] remove global var
			//result->qualified = 1;
			resultlist[mybuffersize - resultposagg] = result;
			//if(aggstate->agg_done == true){
			if(aggstate->agg_done == true){
				//resultposagg++;
				//resultlist[mybuffersize - resultposagg] = NULL;//Taras: set to NULL to know the end! [MYTODO] what happens if mybuffersize < resultposagg
				resultposagg = 0;
				return resultlist;
			}
...
static void
agg_fill_hash_table_qualtuple(AggState *aggstate)
{
	PlanState  *outerPlan;
	ExprContext *tmpcontext;
	AggHashEntry entry;
	...
	for (;;)
	{
		...
		outerslotlist = ExecProcNodeListFull(outerPlan);

		for(i=mybuffersize;i--;){
			outerslot = outerslotlist[i];
...
static void
agg_fill_hash_table_listfull(AggState *aggstate)
{
	PlanState  *outerPlan;
	ExprContext *tmpcontext;
	AggHashEntry entry;
...
static TupleTableSlot **
agg_retrieve_hash_table_qualtuple(AggState *aggstate)
{
	ExprContext *econtext;
	Datum	   *aggvalues;
	bool	   *aggnulls;
	...
	unsigned int aggresultpos = mybuffersize;
	...
	while (!aggstate->agg_done)
	{
		...
		if (ExecQual(aggstate->ss.ps.qual, econtext, false))
		{
			...
			result = ExecProjectBuffer(aggstate->ss.ps.ps_ProjInfo, &isDone,--aggresultpos);
			result->qual = true;
			resultlist[aggresultpos] = result;
...
static TupleTableSlot **
agg_retrieve_hash_table_listfull(AggState *aggstate)
{
	ExprContext *econtext;
	Datum	   *aggvalues;
	bool	   *aggnulls;
...
ExecInitAgg(Agg *node, EState *estate, int eflags)
{
	...
	aggstate->resultlist  = (TupleTableSlot**)MemoryContextAllocZero(CurrentMemoryContext,sizeof(TupleTableSlot*)*mybuffer_size);//Taras: [MYTODO] move to init
	aggstate->ss.ss_ScanTupleSlotList  = (TupleTableSlot**)MemoryContextAllocZero(CurrentMemoryContext,sizeof(TupleTableSlot*)*mybuffer_size);//Taras: [MYTODO] move to init
	aggstate->ss.ps.ps_ResultTupleSlotList  = (TupleTableSlot**)MemoryContextAllocZero(CurrentMemoryContext,sizeof(TupleTableSlot*)*mybuffer_size);//Taras: [MYTODO] move to init
	...
	ExecInitScanTupleSlotBuffer(estate, &aggstate->ss);
	ExecInitResultTupleSlotBuffer(estate, &aggstate->ss.ps);
...
//	ExecAssignProjectionInfo(&aggstate->ss.ps, NULL);
	ExecAssignResultTypeFromTLBuffer(&aggstate->ss.ps);
	ExecAssignProjectionInfoBuffer(&aggstate->ss.ps, NULL);
